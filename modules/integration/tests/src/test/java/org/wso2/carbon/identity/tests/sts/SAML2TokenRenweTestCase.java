...
package org.wso2.carbon.identity.tests.sts;
...
public class SAML2TokenRenweTestCase extends ISIntegrationTest {
	...
	public void runStsClient() {
		...
		String testPath = ProductConstant.getSystemResourceLocation()+File.separator+"stsclient"+File.separator;
		...
		try {
			if (System.getProperty("os.name").toLowerCase().contains("windows")) {
				...
				tempProcess = rt.getRuntime().exec(
						new String[] { "stsclient.bat" }, null, shFile);
			} else {
