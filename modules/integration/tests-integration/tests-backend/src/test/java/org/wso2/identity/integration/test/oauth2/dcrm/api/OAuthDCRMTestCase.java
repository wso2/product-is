...
public class OAuthDCRMTestCase extends ISIntegrationTest {
    ...
    @Test(alwaysRun = true, groups = "wso2.is", priority = 7, description =
            "Create a service provider with saas app property")
    public void testCreateServiceProviderRequestWithSaasApp() throws IOException {
        HttpPost request = new HttpPost(getPath());
        request.addHeader(HttpHeaders.AUTHORIZATION, getAuthzHeader());
        request.addHeader(HttpHeaders.CONTENT_TYPE, OAuthDCRMConstants.CONTENT_TYPE);
        ...
        JSONObject obj = new JSONObject();
        obj.put(OAuthDCRMConstants.CLIENT_NAME, OAuthDCRMConstants.APPLICATION_NAME_WITH_SAAS);
        obj.put(OAuthDCRMConstants.GRANT_TYPES, grantTypes);
        obj.put(OAuthDCRMConstants.REDIRECT_URIS, redirectURI);
        obj.put(OAuthDCRMConstants.EXT_SAAS_APP, true);
