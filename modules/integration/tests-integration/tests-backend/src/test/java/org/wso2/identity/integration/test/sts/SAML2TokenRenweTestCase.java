...
public class SAML2TokenRenweTestCase extends ISIntegrationTest {
	...
	public void testInit() throws Exception {
        ...
        String path = getISResourceLocation()+ isServer.getConfigurationValue("//security/keystore/fileName/text()");
	...
	public void runStsClient() {
		...
		String testPath = getISResourceLocation()+File.separator+"stsclient.stsclient"+File.separator;
		...
		try {
			if (System.getProperty("os.name").toLowerCase().contains("windows")) {
				...
				tempProcess = rt.getRuntime().exec(
						new String[] { "stsclient.stsclient.bat" }, null, shFile);
			} else {
