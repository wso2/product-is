...
public class OAuth2TokenExchangeSubOrganizationTestCase extends AbstractIdentityFederationTestCase {
    ...
    private static final String SUB_ORG_APP2_NAME = "subOrgApp2";
    private static final String TRUSTED_TOKEN_ISSUER_NAME = "trustedTokenIssuer";
    private static final String SUB_ORG_SELF_TRUSTED_IDP_NAME = "subOrgSelfTrustedIssuer";
    ...
    private static final String JWKS_PATH = "https://localhost:9853/oauth2/jwks";
    ...
    private String subOrgApp2ClientId;
    private String subOrgApp2ClientSecret;
    private String subOrgIdpId;
    private String subOrgSelfTrustedIdpId;
    private String secondaryISClientID;
    private String secondaryISClientSecret;
    private String accessTokenFromSecondaryIS;
    private String app1ExchangedToken;
    ...
    public void initTest() throws Exception {
        ...
        // Create applications in sub organization
        createApplicationInSubOrganization();
        createSecondApplicationInSubOrganization();

        // Add trusted token issuer (IDP) in sub organization for secondary IS tokens
        addTrustedTokenIssuerInSubOrganization();

        // Add sub org self-trusted IDP to allow chained token exchange (App1 token → App2 token)
        addSubOrgSelfTrustedIssuerInSubOrganization();
    ...
    public void endTest() throws Exception {
        ...
        try {
            ...
            // Delete IDPs from sub organization
            if (subOrgSelfTrustedIdpId != null) {
                idpMgtRestClient.deleteIdpInOrganization(subOrgSelfTrustedIdpId, switchedM2MToken);
            }
            if (subOrgIdpId != null) {
                idpMgtRestClient.deleteIdpInOrganization(subOrgIdpId, switchedM2MToken);
            }

            // Delete applications from sub organization
            if (subOrgApp2Id != null) {
                oAuth2RestClient.deleteOrganizationApplication(subOrgApp2Id, switchedM2MToken);
            }
            if (subOrgAppId != null) {
    ...
    @Test(groups = "wso2.is", description = "Exchange access token in sub organization via App1")
    public void testTokenExchangeInSubOrganization() throws Exception {

        List<NameValuePair> postParameters = buildTokenExchangeParameters(accessTokenFromSecondaryIS);
        ...
        app1ExchangedToken = responseObject.get("access_token").toString();
        Assert.assertNotNull(app1ExchangedToken, "Exchanged access token via App1 is null.");

        String originalSubject = getTokenSubject(accessTokenFromSecondaryIS);
        String exchangedSubject = getTokenSubject(app1ExchangedToken);

        Assert.assertEquals(exchangedSubject, originalSubject,
                "Subject of the token exchanged via App1 should match the original subject.");
    }

    @Test(groups = "wso2.is",
            description = "Exchange access token in sub organization via App2",
            dependsOnMethods = "testTokenExchangeInSubOrganization")
    public void testTokenExchangeViaSecondAppInSubOrganization() throws Exception {
        ...
        JSONObject responseObject = sendPOSTMessage(subOrgTokenEndpoint, subOrgApp2ClientId, subOrgApp2ClientSecret,
                postParameters);

        String app2ExchangedToken = responseObject.get("access_token").toString();
        Assert.assertNotNull(app2ExchangedToken, "Exchanged access token via App2 is null.");

        String originalSubject = getTokenSubject(accessTokenFromSecondaryIS);
        String exchangedSubject = getTokenSubject(app2ExchangedToken);

        Assert.assertEquals(exchangedSubject, originalSubject,
                "Subject of the token exchanged via App2 should match the original subject.");
    }

    @Test(groups = "wso2.is",
            description = "Chained token exchange: exchange App1 token for an App2 token in sub organization",
            dependsOnMethods = "testTokenExchangeViaSecondAppInSubOrganization")
    public void testChainedTokenExchangeInSubOrganization() throws Exception {

        // Use App1's token (issued by the sub org) as the subject token for App2's token exchange.
        // App2 trusts the sub org token endpoint via subOrgSelfTrustedIdpId, so it can accept
        // tokens originally issued by App1 within the same sub organization.
        List<NameValuePair> postParameters = buildTokenExchangeParameters(app1ExchangedToken);
        ...
        String chainedToken = responseObject.get("access_token").toString();
        Assert.assertNotNull(chainedToken, "Chained exchanged token (App1 -> App2) is null.");

        // The subject should propagate through the token exchange chain.
        String originalSubject = getTokenSubject(accessTokenFromSecondaryIS);
        String chainedSubject = getTokenSubject(chainedToken);

        Assert.assertEquals(chainedSubject, originalSubject,
                "Subject of the chained token should match the original subject from the secondary IS.");
    }
    ...
    /**
     * Create a second application in the sub organization with the token exchange grant.
     */
    private void createSecondApplicationInSubOrganization() throws Exception {

        ApplicationModel application = new ApplicationModel();
        application.setName(SUB_ORG_APP2_NAME);
        application.setDescription("Second application for token exchange testing in sub organization");
        ...
        subOrgApp2Id = oAuth2RestClient.createOrganizationApplication(application, switchedM2MToken);
        Assert.assertNotNull(subOrgApp2Id, "Failed to create second application in sub organization");
        log.info("Created second application with ID: " + subOrgApp2Id);

        OpenIDConnectConfiguration createdOidcConfig = oAuth2RestClient
                .getOIDCInboundDetailsOfOrganizationApp(subOrgApp2Id, switchedM2MToken);
        subOrgApp2ClientId = createdOidcConfig.getClientId();
        subOrgApp2ClientSecret = createdOidcConfig.getClientSecret();

        Assert.assertNotNull(subOrgApp2ClientId, "Sub org App2 client ID is null");
        Assert.assertNotNull(subOrgApp2ClientSecret, "Sub org App2 client secret is null");
    }

    /**
     * Add the sub organization's own token endpoint as a trusted issuer in the sub organization.
     * This enables chained token exchange: tokens issued by App1 (sub org) can be accepted by App2.
     */
    private void addSubOrgSelfTrustedIssuerInSubOrganization() throws Exception {

        String subOrgTokenEndpoint = getRootTenantQualifiedOrgURL(ACCESS_TOKEN_ENDPOINT,
                tenantInfo.getDomain(), subOrgId);
        String subOrgJwksUri = getRootTenantQualifiedOrgURL(JWKS_PATH,
                tenantInfo.getDomain(), subOrgId);

        IdentityProviderPOSTRequest idpPostRequest = new IdentityProviderPOSTRequest();
        idpPostRequest.setName(SUB_ORG_SELF_TRUSTED_IDP_NAME);
        // App1's client ID is used as the alias since App1's token audience is its client ID.
        idpPostRequest.setAlias(subOrgAppClientId);
        idpPostRequest.setIdpIssuerName(subOrgTokenEndpoint);

        IdentityProviderPOSTRequest.Certificate certificate = new IdentityProviderPOSTRequest.Certificate();
        certificate.setJwksUri(subOrgJwksUri);
        idpPostRequest.setCertificate(certificate);

        subOrgSelfTrustedIdpId = idpMgtRestClient.createOrganizationIdentityProvider(idpPostRequest,
                switchedM2MToken);
        Assert.assertNotNull(subOrgSelfTrustedIdpId,
                "Failed to create sub org self-trusted token issuer in sub organization");
        log.info("Created sub org self-trusted IDP with ID: " + subOrgSelfTrustedIdpId);
    }
    ...
    /**
     * Build the standard token exchange POST parameters for a given subject token.
     */
    private List<NameValuePair> buildTokenExchangeParameters(String subjectToken) {

        List<NameValuePair> postParameters = new ArrayList<>();
        postParameters.add(new BasicNameValuePair("subject_token", subjectToken));
        postParameters.add(new BasicNameValuePair("subject_token_type",
