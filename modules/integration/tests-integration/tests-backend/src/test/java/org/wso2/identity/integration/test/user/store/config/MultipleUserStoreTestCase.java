...
public class MultipleUserStoreTestCase extends ISIntegrationTest {
    ...
    public void testAddNewUserStore() throws Exception {
        ...
        assertTrue("User store not deployed within expected time interval",
                userStoreConfigUtils.waitForUserStoreDeployment(userStoreConfigurationClient, domain));
    }

    @SetEnvironment(executionEnvironments = {ExecutionEnvironment.ALL})
    @Test(groups = "wso2.is", description = "Test user login from PRIMARY domain after secondary user store addition.",
            dependsOnMethods = "testAddNewUserStore")
    public void testUserLoginFromPRIMARYAfterUserStoreAddition() throws RemoteException,
            LoginAuthenticationExceptionException {

        AuthenticatorClient loginLogoutUtil = new AuthenticatorClient(backendURL);
        String sessionCookie = loginLogoutUtil
                .login("PRIMARY" + "/" + "admin", "admin", "localhost");
        assertNotNull("User not logged in", sessionCookie);
    }
    ...
    public void testDisableUserStore() throws Exception {
        ...
        assertTrue("User store is active even after 15 min", userStoreConfigUtils
                .waitForUserStoreUnDeployment(userStoreConfigurationClient, domain));

        AuthenticatorClient loginLogoutUtil =
                new AuthenticatorClient(backendURL);

        assertNull("User is logged in after disabling the user store",
                loginLogoutUtil.login(domain + "/" + TEST_SECONDARY_USER, TEST_SECONDARY_USER, "localhost"));
    }
    ...
    public void testUpdateUserStore() throws Exception {
        ...
        UserStoreDTO userStoreDTO = userStoreConfigurationClient.getSecondaryRealmConfigurations()[0];
        userStoreDTO.setDomainId(newDomain);

        userStoreConfigurationClient.updateUserStoreWithDomainName(domain, userStoreDTO);
        assertTrue("User store not deployed within expected time interval",
                userStoreConfigUtils.waitForUserStoreDeployment(userStoreConfigurationClient, newDomain));
    }
    ...
    public void testDeleteStoreAndUsers() throws Exception {
        ...
        assertTrue("User store was not deleted successfully", userStoreConfigUtils.waitForUserStoreUnDeployment
                (userStoreConfigurationClient, newDomain));
    }
    ...
    public void cleanupTest() throws Exception {
        ...
        if (userStoreConfigUtils.waitForUserStoreDeployment(userStoreConfigurationClient, newDomain)) {
            userStoreConfigurationClient.deleteUserStoresSet(new String[]{newDomain});
