...
public class Oauth2JWKSEndpointTestCase {
    ...
    private void getPublicKeySet(String jwksEndpoint) throws IOException, JSONException {
        ...
        // Find the RSA key in the JSON array to validate existing assertions
        JSONObject rsaKey = null;
        for (int i = 0; i < publicKeySet.getJSONArray(KEYS).length(); i++) {
            JSONObject keyObj = publicKeySet.getJSONArray(KEYS).getJSONObject(i);
            if (keyObj.has(KEY_TYPE) && "RSA".equals(keyObj.getString(KEY_TYPE))) {
                rsaKey = keyObj;
                break;
            }
        }
        Assert.assertNotNull(rsaKey, "RSA key not found in the JWKS response");

        // Validate keys in the JSON object
        validateKey(rsaKey, KEY_TYPE, "The public key type");
        validateKey(rsaKey, EXPONENT, "The exponent value of the public key");
        validateKey(rsaKey, USAGE, "Usage of the key");
        validateKey(rsaKey, THUMBPRINT, "The thumbprint of the certificate");
        validateKey(rsaKey, ALGORITHM, "The algorithm");
        validateKey(rsaKey, MODULUS, "The modulus value of the public key");
        ...
        if (rsaKey.has(X5C) && rsaKey.has(X5T_SHA256)) {
            ...
            Assert.assertEquals(String.valueOf(rsaKey.get(X5T_SHA256)),
