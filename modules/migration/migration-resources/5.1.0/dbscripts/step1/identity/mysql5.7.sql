DROP PROCEDURE IF EXISTS drop_idx_at_ck_au;
CREATE PROCEDURE drop_idx_at_ck_au() BEGIN IF((SELECT COUNT(*) AS index_exists FROM information_schema.statistics WHERE TABLE_SCHEMA = DATABASE() and table_name = 'IDN_OAUTH2_ACCESS_TOKEN' AND index_name = 'IDX_AT_CK_AU') > 0) THEN SET @s = CONCAT('DROP INDEX ' , 'IDX_AT_CK_AU' , ' ON ' , 'IDN_OAUTH2_ACCESS_TOKEN'); PREPARE stmt FROM @s; EXECUTE stmt; END IF; END;

DROP PROCEDURE IF EXISTS drop_idx_at_conk_utype;
CREATE PROCEDURE drop_idx_at_conk_utype() BEGIN IF((SELECT COUNT(*) AS index_exists FROM information_schema.statistics WHERE TABLE_SCHEMA = DATABASE() and table_name = 'IDN_OAUTH2_ACCESS_TOKEN' AND index_name = 'IDX_OAUTH_ACCTKN_CONK_UTYPE') > 0) THEN SET @s = CONCAT('DROP INDEX ' , 'IDX_OAUTH_ACCTKN_CONK_UTYPE' , ' ON ' , 'IDN_OAUTH2_ACCESS_TOKEN'); PREPARE stmt FROM @s; EXECUTE stmt; END IF; END;
...
CALL drop_idx_at_ck_au();
CALL drop_idx_at_conk_utype();
