...
BEGIN
  ...
  begin
    select constraint_name into con_name from all_constraints where table_name='IDN_AUTH_SESSION_STORE' AND owner=databasename AND constraint_type = 'P';
  ...
  begin
    select constraint_name into con_name from all_constraints where table_name='IDN_OAUTH2_AUTHORIZATION_CODE' AND owner=databasename AND constraint_type = 'P';
...
/
ALTER TABLE IDN_OAUTH2_AUTHORIZATION_CODE ADD CODE_ID VARCHAR (255) NOT NULL
/
...
-- Removing all the persisted session data
