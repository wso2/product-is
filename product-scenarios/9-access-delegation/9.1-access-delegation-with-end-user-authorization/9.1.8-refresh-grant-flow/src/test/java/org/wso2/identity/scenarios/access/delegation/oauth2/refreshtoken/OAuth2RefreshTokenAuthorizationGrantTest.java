 ...
 */
package org.wso2.identity.scenarios.access.delegation.oauth2.refreshtoken;
...
/**
 * This test class tests the refresh token retrieval during authorization code grant flow.
 */
public class OAuth2RefreshTokenAuthorizationGrantTest extends ScenarioTestBase {
    ...
    @Factory(dataProvider = "oAuthRefreshTokenAuthorizationGrantConfigProvider")
    public OAuth2RefreshTokenAuthorizationGrantTest(String dcrRequestFile, String appCreatorUsername, String appCreatorPassword,
    ...
    @DataProvider(name = "oAuthRefreshTokenAuthorizationGrantConfigProvider")
    private static Object[][] oAuthRefreshTokenAuthorizationGrantConfigProvider() throws Exception {

        return new Object[][]{
                {
                        "dcr-request-2.json", ADMIN_USERNAME, ADMIN_PASSWORD, ADMIN_USERNAME, ADMIN_PASSWORD,
                        SUPER_TENANT_DOMAIN
                }
        };
    }
    ...
    @Test(description = "9.1.8.2.7",
            dependsOnMethods = "introspectAccessToken")
    public void sendValidRefreshTokenRequest() throws Exception {

        HttpResponse response = oAuth2CommonClient.sendRefreshTokenRequest(clientId, clientSecret, refreshToken, null);
        JSONObject responseJSON = httpCommonClient.getJSONFromResponse(response);
        refreshToken = responseJSON.get(OAuth2Constants.TokenResponseElements.REFRESH_TOKEN).toString();
    ...
    public void sendInvalidRefreshTokenRequest() throws Exception {

        invalidRefreshToken = refreshToken + "==";
        HttpResponse response = oAuth2CommonClient.sendRefreshTokenRequest(clientId, clientSecret, invalidRefreshToken, null);
        assertNotEquals(response.getStatusLine().getStatusCode(), HttpStatus.SC_OK, "An invalid refresh token is not " +
