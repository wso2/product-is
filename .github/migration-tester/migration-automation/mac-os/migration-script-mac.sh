...
chmod +x update-GA-pack-mac.sh
sh update-GA-pack-mac.sh "$email" "$password" "current"
wait $!
...
# Download needed wso2IS zip
chmod +x download-GA-pack.sh
bash download-GA-pack.sh "$keyJsonFile" "$os" "$currentVersion" "migrating"

cd "$AUTOMATION_HOME_MAC"

# Update downloaded GA pack
chmod +x update-GA-pack-mac.sh
sh update-GA-pack-mac.sh "$email" "$password" "migrating"
wait $!
