...
disable_test() {
  ...
  sed -i.bak "s/name=\"$test_name\"/& enabled=\"false\"/" product-is-2/modules/integration/tests-integration/tests-backend/src/test/resources/testng.xml
}
...
git clone https://github.com/wso2/product-is product-is-2
...
if [ "$REPO" = "product-is" ]; then
  ...
  VERSION_PROPERTY=$(python version_property_finder.py $REPO product-is-2 2>&1)
