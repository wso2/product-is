...
then
  ...
  python3 $CONFORMANCE_SUITE_PATH/scripts/run-test-plan.py fapi1-advanced-final[server_metadata=static][client_registration=static_client] $PATH_TO_SCRIPTS/fapi/IS_config_fapi.json 2>&1 | tee fapi-certification-test-plan-log.txt
  echo
else
